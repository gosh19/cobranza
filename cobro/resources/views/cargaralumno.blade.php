...
<script type="text/javascript">
  ...
  $(document).ready(function(){
    ...
    $('.tipo').on('change', function() {
      if ((this.value == "debito")||(this.value == "efectivo")) {
        $("#telefono").slideDown();
        $("#credito").slideUp();
      }
...
@section('formulario')
  ...
  <div class="d-flex justify-content-center">
    <form class="" action="/cargarcobro" method="get">
      ...
      <div id="telefono" style="width:100%; background:#2E9AFE; padding:10px; border-radius:10px;" >
        <input type="text" name="telefono" value="" placeholder="Telefono"><br>
        <input type="radio" name="cuotas" value="6" checked style="width:20px;">6 Cuotas<br>
      </div>
      <div id="credito" style="width:100%; background:#2E9AFE; padding:10px; border-radius:10px;">
        <select class="" name="cant_cuotas">
          @for ($i=12; $i > 0; $i--)
            <option value="{{$i}}">{{$i}} Couta(s)</option>
          @endfor
        </select>
      </div>
      <div id="no-titular">
